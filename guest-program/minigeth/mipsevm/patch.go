...
func LoadMappedFile(s *State, fn string, base uint32) (*State, error) {
	dat, err := os.ReadFile(fn)
	if err != nil {
		return nil, fmt.Errorf("load file %s failed", fn)
	}
	s.Memory.SetMemoryRange(base, bytes.NewReader(dat))
	return s, nil
}
